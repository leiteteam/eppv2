...
public class JepayDatabase {
    ...
    private static void upgradeDatabase(DbUtils dbUtils, int oldVer, int newVer) {
        ...
        try {
            ...
            //数据库版本为3时，因表结构发生变化，重新启用数据库升级策略
            if (newVer == 3) {
                //本次数据库升级需修改表字段，需先删除旧track表
                dbUtils.dropTable(Track.class);
                com.androidcat.utilities.LogUtil.e(TAG, "----删除旧版Track表，新增id字段----");
            }
    ...
    public String updateTaskDataWithTrack(String taskid) {
        ...
        String track = new Gson().toJson(tracks, new TypeToken<List<Track>>() {
        }.getType());
        return track;
    }
  ...
  public boolean updateTaskDataToUploaded(String taskId) {
    ...
    public boolean delTrackByTaskid(String taskid) {
        if (mDbUtils == null) {
            return false;
        }
        try {
            mDbUtils.deleteAll(getTrackList(taskid));
            return true;
        } catch (DbException e) {
            e.printStackTrace();
            return false;

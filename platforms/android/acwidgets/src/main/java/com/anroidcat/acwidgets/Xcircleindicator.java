...
public class Xcircleindicator extends View {
    ...
    public Xcircleindicator(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            int fillColor = a.getColor(R.styleable.XCircleIndicator_fillColor1,
                    0xFFFFFFFF);
            int strokeColor = a.getColor(
                    R.styleable.XCircleIndicator_strokeColor1, 0xFFFFFFFF);
            // Retrieve the radius
            radius = (int) a.getDimension(R.styleable.XCircleIndicator_radius1,
                    radius);
            circleInterval = (int) a.getDimension(
                    R.styleable.XCircleIndicator_circleInterval, radius);

            width = (int) a.getDimension(
                    R.styleable.XCircleIndicator_circleWidth, radius);
    ...
    public Xcircleindicator(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int iLoop = 0; iLoop < pageTotalCount; iLoop++) {
            ...
            //canvas.drawRect(getPaddingLeft() + (iLoop * (width + circleInterval)), getPaddingTop(), getPaddingTop() + width, getPaddingTop() + 2 * radius, mPaintStroke);
        ...
        //canvas.drawRect(getPaddingLeft() + (currentPage * (width + circleInterval)), getPaddingTop(), getPaddingTop() + width, getPaddingTop() + 2 * radius, mPaintFill);
    }

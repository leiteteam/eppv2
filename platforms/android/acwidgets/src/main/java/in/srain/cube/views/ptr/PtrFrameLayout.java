...
import static com.anroidcat.acwidgets.R.styleable;
...
public class PtrFrameLayout extends ViewGroup {
    ...
    public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (arr != null) {
            mHeaderId = arr.getResourceId(styleable.PtrFrameLayout_ptr_header, mHeaderId);
            mContainerId = arr.getResourceId(styleable.PtrFrameLayout_ptr_content, mContainerId);
            mFooterId = arr.getResourceId(styleable.PtrFrameLayout_ptr_footer, mFooterId);

            mPtrIndicator.setResistanceHeader(arr.getFloat(styleable.PtrFrameLayout_ptr_resistance, mPtrIndicator.getResistanceHeader()));
            mPtrIndicator.setResistanceFooter(arr.getFloat(styleable.PtrFrameLayout_ptr_resistance, mPtrIndicator.getResistanceFooter()));
            mPtrIndicator.setResistanceHeader(arr.getFloat(styleable.PtrFrameLayout_ptr_resistance_header, mPtrIndicator.getResistanceHeader()));
            mPtrIndicator.setResistanceFooter(arr.getFloat(styleable.PtrFrameLayout_ptr_resistance_footer, mPtrIndicator.getResistanceFooter()));

            mDurationToBackHeader = arr.getInt(styleable.PtrFrameLayout_ptr_duration_to_back_refresh, mDurationToCloseHeader);
            mDurationToBackFooter = arr.getInt(styleable.PtrFrameLayout_ptr_duration_to_back_refresh, mDurationToCloseHeader);
            mDurationToBackHeader = arr.getInt(styleable.PtrFrameLayout_ptr_duration_to_back_header, mDurationToCloseHeader);
            mDurationToBackFooter = arr.getInt(styleable.PtrFrameLayout_ptr_duration_to_back_footer, mDurationToCloseHeader);

            mDurationToCloseHeader = arr.getInt(styleable.PtrFrameLayout_ptr_duration_to_close_either, mDurationToCloseHeader);
            mDurationToCloseFooter = arr.getInt(styleable.PtrFrameLayout_ptr_duration_to_close_either, mDurationToCloseFooter);
            mDurationToCloseHeader = arr.getInt(styleable.PtrFrameLayout_ptr_duration_to_close_header, mDurationToCloseHeader);
            mDurationToCloseFooter = arr.getInt(styleable.PtrFrameLayout_ptr_duration_to_close_footer, mDurationToCloseFooter);

            float ratio = mPtrIndicator.getRatioOfHeaderToHeightRefresh();
            ratio = arr.getFloat(styleable.PtrFrameLayout_ptr_ratio_of_header_height_to_refresh, ratio);
            mPtrIndicator.setRatioOfHeaderHeightToRefresh(ratio);

            mKeepHeaderWhenRefresh = arr.getBoolean(styleable.PtrFrameLayout_ptr_keep_header_when_refresh, mKeepHeaderWhenRefresh);

            mPullToRefresh = arr.getBoolean(styleable.PtrFrameLayout_ptr_pull_to_fresh, mPullToRefresh);

            mMode = getModeFromIndex(arr.getInt(styleable.PtrFrameLayout_ptr_mode, 4));

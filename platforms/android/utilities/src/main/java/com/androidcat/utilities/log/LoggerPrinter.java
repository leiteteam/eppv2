...
public final class LoggerPrinter implements Printer {
  ...
  @Override public Settings init(String tag, Context context) {
    if (tag == null) {
      throw new NullPointerException("tag may not be null");
    }
    if (tag.trim().length() == 0) {
  ...
  @Override public void file(String tag,String message) {
    log(FILE,tag, message);
  }
  ...
  public void xmlToFile(String xml){
    if (TextUtils.isEmpty(xml)) {
      d("Empty/Null xml content");
      return;
    }
    try {
      ...
      transformer.setOutputProperty("{http://xml.apache.org/xslt}indent-amount", "2");
      transformer.transform(xmlInput, xmlOutput);
      file(xmlOutput.getWriter().toString().replaceFirst(">", ">\n"));
    } catch (TransformerException e) {
      e(e.getCause().getMessage() + "\n" + xml);
    }
  }
  ...
  @Override
  public void file(String message, boolean async) {
    log(FILE, message, async);
  }
   ...
   */
  private synchronized String log(int logType, String msg, Object... args) {
    ...
    if (logType != FILE){
      stringBuilder.append(logTopBorder(logType, tag));
      stringBuilder.append("\n");
      stringBuilder.append(logHeaderContent(logType, tag, methodCount));
    }else {
      //do nothing...
    }
    fileLog.append(String.format(" Time: " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SSS").format(new Date()))+"--"+Thread.currentThread().getName());
    fileLog.append("\n");
    ...
    if (length <= CHUNK_SIZE) {
      if (methodCount > 0) {
        if (logType != FILE){
          stringBuilder.append(logDivider(logType, tag));
          stringBuilder.append("\n");
        }
      }
      String content = logContent(logType, tag, message);
      stringBuilder.append(content);
      fileLog.append(content);
      if (logType != FILE){
        stringBuilder.append(logBottomBorder(logType, tag));
      }
      //TODO
      String logData = fileLog.toString();
      if (settings.isWriteToFile() || logType == FILE){
        LogFileTool.getInstance().logAsync(logData);
      }
    ...
    for (int i = 0; i < length; i += CHUNK_SIZE) {
      ...
      String content = logContent(logType, tag, new String(bytes, i, count));
      stringBuilder.append(content);
      fileLog.append(content);
    }
    if (logType != FILE){
  ...
  private synchronized String log(int logType, String msg, boolean async) {
    if (settings.getLogLevel() == LogLevel.NONE) {
      return "";
    }
    StringBuilder stringBuilder = new StringBuilder();
    ...
    if (length <= CHUNK_SIZE) {
      ...
      if (settings.isWriteToFile() || logType == FILE) {
        if (async) {
          ...
          LogFileTool.getInstance().log(logData);
        }
      }
      return logData;
    }
   ...
   * This method is synchronized in order to avoid messy of logs' order.
   */
  private synchronized String log(int logType,String tag, String msg, Object... args) {
    if (settings.getLogLevel() == LogLevel.NONE) {
      return "";
    }
    StringBuilder stringBuilder = new StringBuilder();
  ...
  private String logTopBorder(int logType, String tag) {
  ...
  private String logHeaderContent(int logType, String tag, int methodCount) {
    ...
    sb.append(logChunk(logType, tag, String.format(HORIZONTAL_DOUBLE_LINE + " Time: " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SSS").format(new Date()))));
    sb.append("\n");
    if (settings.isShowThreadInfo()) {
      sb.append(logChunk(logType, tag, HORIZONTAL_DOUBLE_LINE + " Thread: " + Thread.currentThread().getName()));
      sb.append("\n");
      sb.append(logDivider(logType, tag));
      sb.append("\n");
    }
    String level = "";
    ...
    for (int i = methodCount; i > 0; i--) {
      ...
      builder.append("| ")
          ...
          .append(")");
      level += "   ";
      sb.append(logChunk(logType, tag, builder.toString()));
      sb.append("\n");
    }
    return sb.toString();
  }

  private String logBottomBorder(int logType, String tag) {
    return logChunk(logType, tag, BOTTOM_BORDER);
  }

  private String logDivider(int logType, String tag) {
    return logChunk(logType, tag, MIDDLE_BORDER);
  }

  private String logContent(int logType, String tag, String chunk) {
    StringBuilder sb = new StringBuilder();
  ...
  private String logChunk(int logType, String tag, String chunk) {
